...
class TextPredictionModel:
    ...
    def predict(self, text_list, top_k=5):
        ...
        predictions_indexes = self.model.predict(embedded_texts)
        # TODO: CODE HERE
        # from tags indexes compute top_k tags for each text
        predictions = []
        for pred_indexes in predictions_indexes:
            top_k_indexes = argsort(pred_indexes)[-top_k:][::-1]
            top_k_labels = [self.labels_index_inv[ind] for ind in top_k_indexes]
